...
export default function Shop() {
  ...
  return (
    <>
      <TitlePage title={t.shop.pageTitle} />
      <Header />
      <section className='shop-header mt-7 md:mt-0 h-60 xs:h-80 md:h-screen bg-shop-header bg-no-repeat bg-cover bg-[center_top] xs:bg-[left_top]'>
        <div className="container flex items-center h-full">
          <h1 className='text-2xl xs:text-3xl sm:text-4xl md:text-5xl ipad:text-6xl select-none font-morabba-bold text-white'>{t.shop.pageTitle}</h1>
        </div>
      </section>
      <section className='shop-about my-10 md:my-16'>
        <div className="container">
          <div className="bg-white dark:bg-zinc-700 dark:text-white rounded-lg p-6 md:p-8">
            <h2 className='text-2xl md:text-3xl font-morabba-medium border-b-2 border-b-black dark:border-b-orange-300 w-max dark:text-orange-300 pb-1'>{t.shop.aboutProductsTitle}</h2>
            <p className='mt-5 leading-relaxed'>
              {t.shop.aboutProductsText}
              <br /><br />
              {t.shop.aboutProductsText2}
            </p>
          </div>
        </div>
      </section>
        ...
        <div className="container">
          ...
          >
            <select className='w-32 xs:w-52 py-2 text-white text-[13px] xs:text-base bg-gray-400 dark:bg-gray-500 rounded-md focus:outline-none'>
              <option value="جدید ترین">{t.shop.newest}</option>
              <option value="ارزان ترین">{t.shop.cheapest}</option>
              <option value="گران ترین">{t.shop.mostExpensive}</option>
              <option value="تخفیفی ترین">最大割引 · Biggest Discount</option>
              <option value="محبوب ترین">人気 · Most Popular</option>
            </select>
          </SectionHeader>
