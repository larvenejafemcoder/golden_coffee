...
export default function Home() {
  ...
  const lastProductsRef = useRef<HTMLDivElement>(null)

  const scrollToLastProductsSection = () => {
    let lastProductsSectionSpaceTop = (lastProductsRef.current!.firstChild as HTMLElement).offsetTop
        ...
        <section
          ...
          <div className="container relative overflow-y-hidden flex items-center justify-end h-full md:min-h-screen">
            <div className="select-none text-white">
              <h2 className="font-morabba-bold text-xl xs:text-3xl md:text-5xl/[62px] mb-0.5 md:mb-2">
                {t.home.heroTitle1}<br />{t.home.heroTitle2}
              </h2>
              <span className="font-morabba text-lg xs:text-2xl md:text-4xl/[64px]">{t.home.heroSubtitle}</span>
              ...
              <p className="max-w-44 xs:max-w-52 sm:max-w-64 md:max-w-[420px] text-xs md:text-xl">
                Everyone has surely heard the name Arabica. Arabica is one of the coffee species grown in various regions of the coffee belt.
              </p>
            </div>
