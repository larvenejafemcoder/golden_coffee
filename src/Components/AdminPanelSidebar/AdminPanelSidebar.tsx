...
export default function AdminPanelSidebar({ setShow }: AdminPanelSidebarProps) {
  const { t } = useTranslation()

  const homePage = useRef<HTMLAnchorElement>(null)

  const isActiveMenu = (event: React.MouseEvent<HTMLLIElement>) => {
    if (setShow) {
      setShow(false)
    }

    if ((event.target as HTMLElement).textContent === t.adminSidebar.mainPage) {
  ...
  const showAlert = () => {
    Swal.fire({
      text: t.adminSidebar.logoutConfirm,
      icon: 'success',
